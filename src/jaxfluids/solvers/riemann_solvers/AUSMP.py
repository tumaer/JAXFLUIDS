...
class AUSMP(RiemannSolver):
    ...
    def _solve_riemann_problem_xi_single_phase(
        ...
        if self.interface_speed_of_sound == "CRITICAL": # Eq. 40
            a_star_L  = jnp.sqrt(2.0 * (gamma - 1.0) / (gamma + 1.0) * phi_L[self.ids_energy] / phi_L[self.ids_mass])
            a_star_R  = jnp.sqrt(2.0 * (gamma - 1.0) / (gamma + 1.0) * phi_R[self.ids_energy] / phi_R[self.ids_mass])
        ...
        # Eq. 19
        M_plus  = jnp.where(jnp.abs(M_l) >= 1, 0.5 * (M_l + jnp.abs(M_l)),  0.25 * jnp.square(M_l + 1.0) + self.beta * jnp.square(M_l * M_l - 1.0))
        M_minus = jnp.where(jnp.abs(M_r) >= 1, 0.5 * (M_r - jnp.abs(M_r)), -0.25 * jnp.square(M_r - 1.0) - self.beta * jnp.square(M_r * M_r - 1.0))  
        ...
        # Eq. 21
        P_plus  = jnp.where(jnp.abs(M_l) >= 1.0, 0.5 * (1 + jnp.sign(M_l)), 0.25 * jnp.square(M_l + 1.0) * (2.0 - M_l) + self.alpha * M_l * jnp.square(M_l * M_l - 1.0))
        P_minus = jnp.where(jnp.abs(M_r) >= 1.0, 0.5 * (1 - jnp.sign(M_r)), 0.25 * jnp.square(M_r - 1.0) * (2.0 + M_r) - self.alpha * M_r * jnp.square(M_r * M_r - 1.0))  
        # Eq. A2
        pressure_ausm = P_plus * primitives_L[self.ids_energy] + P_minus * primitives_R[self.ids_energy]
    ...
    def get_phi(self, primitives: Array, conservatives: Array) -> Array:
        ...
        :rtype: Array
        """
        rho =  conservatives[self.ids_mass] 
        rhou = conservatives[self.ids_velocity[0]] 
        rhov = conservatives[self.ids_velocity[1]] 
        rhow = conservatives[self.ids_velocity[2]] 
        ht   = conservatives[self.ids_energy] + primitives[self.ids_energy]
