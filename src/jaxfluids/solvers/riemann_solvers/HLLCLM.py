...
class HLLCLM(RiemannSolver):
    ...
    def _solve_riemann_problem_xi_single_phase(
        ...
        # Toro 10.73
        pre_factor_L = (wave_speed_simple_L - primitives_L[self.ids_velocity[axis]]) / (wave_speed_simple_L - wave_speed_contact) * primitives_L[self.ids_mass]
        pre_factor_R = (wave_speed_simple_R - primitives_R[self.ids_velocity[axis]]) / (wave_speed_simple_R - wave_speed_contact) * primitives_R[self.ids_mass]

        u_star_L = [pre_factor_L, pre_factor_L, pre_factor_L, pre_factor_L, pre_factor_L * (conservatives_L[self.ids_energy] / conservatives_L[self.ids_mass] + (wave_speed_contact - primitives_L[self.ids_velocity[axis]]) * (wave_speed_contact + primitives_L[self.ids_energy] / primitives_L[self.ids_mass] / (wave_speed_simple_L - primitives_L[self.ids_velocity[axis]]) )) ]
        ...
        u_star_R = [pre_factor_R, pre_factor_R, pre_factor_R, pre_factor_R, pre_factor_R * (conservatives_R[self.ids_energy] / conservatives_R[self.ids_mass] + (wave_speed_contact - primitives_R[self.ids_velocity[axis]]) * (wave_speed_contact + primitives_R[self.ids_energy] / primitives_R[self.ids_mass] / (wave_speed_simple_R - primitives_R[self.ids_velocity[axis]]) )) ]
        ...
        fluxes_xi = \
            0.5 * (1 + jnp.sign(wave_speed_simple_L)) * fluxes_L \
    ...
    def _solve_riemann_problem_xi_diffuse_five_equation(
        ...
        U_star_L = [
            *primitives_L[self.s_mass],
            ...
            rho_L,
            rho_L,
            conservatives_L[self.ids_energy] + (wave_speed_contact - u_L) * (rho_L * wave_speed_contact + p_L_prime / (wave_speed_simple_L - u_L) ),
        ...
        U_star_R = [
            *primitives_R[self.s_mass],
            ...
            rho_R,
            rho_R,
            conservatives_R[self.ids_energy] + (wave_speed_contact - u_R) * (rho_R * wave_speed_contact + p_R_prime / (wave_speed_simple_R - u_R) ),
