...
from jaxfluids.math.fft import parallel_fft, rfft3D, real_wavenumber_grid, \
    wavenumber_grid_parallel
...
class TurbulentStatisticsManager:
    ...
    TODO think about merging with the different online stats
    """
    ...
    def hit_statistics(self, primitives: Array) -> Dict:
        ...
        # Sum over ensemble axis (axis=0) and 
        # three spatial dimension (-3,-2,-1)
        s_axis = (0,-1,-2,-3)

        def hit_mean(quant: Array) -> Array:
            quant_mean = jnp.mean(quant, axis=s_axis)
            if is_parallel:
                quant_mean = jax.lax.pmean(quant_mean, axis_name="i")
            return quant_mean

        assert_str = (
            "To evaluate HIT statistics, provide a single snapshot (5,Nx,Ny,Nz)"
        ...
        velocity_mean = hit_mean(velocity)
        velocity = velocity - velocity_mean[...,None,None,None]
        ...
        density_mean = hit_mean(density)
        pressure_mean = hit_mean(pressure)
        temperature_mean = hit_mean(temperature)

        density_rms = jnp.sqrt(hit_mean(jnp.square(density - density_mean)))
        pressure_rms = jnp.sqrt(hit_mean(jnp.square(pressure - pressure_mean)))
        temperature_rms = jnp.sqrt(hit_mean(jnp.square(temperature - temperature_mean)))

        dynamic_viscosity = self.material_manager.get_dynamic_viscosity(temperature, None)
        dynamic_viscosity_mean = hit_mean(dynamic_viscosity)
        kinematic_viscosity_mean = dynamic_viscosity_mean / density_mean

        # KINETIC ENERGY AND U_RMS
        velocity_squared = jnp.sum(velocity * velocity, axis=-4)
        u_rms = jnp.sqrt(hit_mean(velocity_squared) / 3)
        q_rms = jnp.sqrt(3) * u_rms
        tke = 1.5 * u_rms**2
        TKE = 0.5 * hit_mean(density * velocity_squared)

        # TURBULENT MACH NUMBER
        speed_of_sound = self.material_manager.get_speed_of_sound(
            pressure=pressure, density=density)
        speed_of_sound_mean_1 = self.material_manager.get_speed_of_sound(
            pressure=pressure_mean, density=density_mean)
        speed_of_sound_mean_2 = hit_mean(speed_of_sound)

        Ma_t_urms = u_rms / speed_of_sound_mean_1
        Ma_t_qrms = q_rms / speed_of_sound_mean_1
        # Check this quantity
        Ma_rms = jnp.sqrt(hit_mean(velocity_squared / speed_of_sound))

        # SHEAR TENSOR, STRAIN RATE & DISSIPATION
        # Shear tensor
        if is_parallel:
            dui_dj = _calculate_sheartensor_spectral_parallel(
                velocity_hat, k_field,
                split_factors_out, split_axis_in)
        else:
            dui_dj = _calculate_sheartensor_spectral(velocity_hat, k_field)
        # Strain rate
        S_ij = 0.5 * (dui_dj + jnp.swapaxes(dui_dj, -4, -5)) 
        # Viscous stress
        sigma_ij = 2 * dynamic_viscosity * S_ij \
            - 2/3 * dynamic_viscosity * (S_ij[...,0:1,0:1,:,:,:] \
                + S_ij[...,1:2,1:2,:,:,:] + S_ij[...,2:3,2:3,:,:,:])

        # Dissipation according to Eq. (62) in Erlebacher et al. 1992
        # Toward the large-eddy simulation of compressible turbulent flows
        # eps = 2 * kinematic_viscosity_mean * hit_mean(jnp.sum(S_ij * S_ij, axis=(-4,-5)))

        # Dissipation according to Xie et al. 2019
        # Artificial neural network mixed model for large eddy simulation of compressible isotropic turbulence
        eps = hit_mean(jnp.sum(S_ij * sigma_ij, axis=(-4,-5)) / density)

        # VORTICITY
        if is_parallel:
            vorticity = calculate_vorticity_spectral_parallel(
                velocity_hat, k_field,
                split_factors_out, split_axis_in)
        else:
            vorticity = calculate_vorticity_spectral(velocity_hat, k_field)
        enstrophy_mean = hit_mean(jnp.sum(vorticity * vorticity, axis=-4)) # omega_i omega_i
        vorticity_rms = jnp.sqrt(1/3 * enstrophy_mean)

        # DILATATION
        if is_parallel:
            dilatation = calculate_dilatation_spectral_parallel(
                velocity_hat, k_field, 
                split_factors_out, split_axis_in)
        else:
            dilatation = calculate_dilatation_spectral(velocity_hat, k_field)
        dilatation_mean = hit_mean(dilatation)

        dilatation_std = jnp.sqrt(hit_mean(jnp.square(dilatation - dilatation_mean)))
        divergence_rms_spec = jnp.sqrt(hit_mean(jnp.square(dilatation)))
        divergence_rms_fd = jnp.sqrt(hit_mean(jnp.square(
            jnp.trace(dui_dj, axis1=-4, axis2=-5))))
        ...
        if is_parallel:
            ...
            rho_spec = jnp.mean(vmap_energy_spectrum_physical_parallel(density, split_factors, is_scalar_field=True), axis=0)
            ...
            rho_spec = jnp.mean(vmap_energy_spectrum_physical(density, is_scalar_field=True), axis=0)
        ...
        f_corr = []
        g_corr = []
        u1u1_bar = hit_mean(velX * velX)
        u2u2_bar = hit_mean(velY * velY)
        if is_parallel:
            if split_axis_in == 0:
                for ii in range(number_of_cells[1] // 2):
                    f_corr_ii = hit_mean(velY * jnp.roll(velY, shift=ii, axis=-2)) / u2u2_bar # longitudinal
                    g_corr_ii = hit_mean(velX * jnp.roll(velX, shift=ii, axis=-2)) / u1u1_bar # transversal
                    f_corr.append(f_corr_ii)
                    g_corr.append(g_corr_ii)
            else:
                for ii in range(number_of_cells[0] // 2):
                    f_corr_ii = hit_mean(velX * jnp.roll(velX, shift=ii, axis=-3)) / u1u1_bar # longitudinal
                    g_corr_ii = hit_mean(velY * jnp.roll(velY, shift=ii, axis=-3)) / u2u2_bar # transversal
                    f_corr.append(f_corr_ii)
                    g_corr.append(g_corr_ii)
        else:
            for ii in range(number_of_cells[0] // 2):
        ...
        int_Ek = jsp.integrate.trapezoid(ek_spec)
        int_Ek_over_k = jsp.integrate.trapezoid(ek_spec / (k_vec + 1e-10))
        L_I = 0.75 * jnp.pi * int_Ek_over_k / int_Ek

        # Longitudinal (lambda_f) and lateral (lambda_g) Taylor length scale
        # lambda_f = jnp.sqrt(hit_mean(velX * velX) / hit_mean(dui_dj[...,0,0,:,:,:] * dui_dj[...,0,0,:,:,:]))
        lambda_f = jnp.sqrt(hit_mean(velX * velX + velY * velY + velZ * velZ) \
            / hit_mean(dui_dj[...,0,0,:,:,:] * dui_dj[...,0,0,:,:,:] + dui_dj[...,1,1,:,:,:] * dui_dj[...,1,1,:,:,:] \
            + dui_dj[...,2,2,:,:,:] * dui_dj[...,2,2,:,:,:]))
        lambda_g = jnp.sqrt(hit_mean(velX * velX) / hit_mean(dui_dj[...,0,1,:,:,:] * dui_dj[...,0,1,:,:,:]))

        # Kolmogorov
        eta = (kinematic_viscosity_mean**3 / eps)**0.25
        eta_kmax = eta * Nx / 2
        eta_over_dx = eta / (2 * jnp.pi / Nx)
        ...
        # REYNOLDS NUMBER
        Re_lambda = lambda_f * u_rms / kinematic_viscosity_mean
        Re_lambda_g = lambda_g * u_rms / kinematic_viscosity_mean
        # Spyropoulos et al. 1996
        Re_turb = q_rms**4 / (eps * kinematic_viscosity_mean) 
        # Sarkar et al. 1991
        Re_lambda_enstrophy = q_rms**2 / (jnp.sqrt(enstrophy_mean) * kinematic_viscosity_mean)
        Re_int = u_rms * L_I / kinematic_viscosity_mean
        Re_L = tke**2 / (eps * kinematic_viscosity_mean)
        Re_T = u_rms * L_11 / kinematic_viscosity_mean
        ...
        turb_stats_dict = {
            ...
            "VELOCITY_SCALES": {
                ...
                "MU_MEAN": dynamic_viscosity_mean,
                "VEL_X_MEAN": velocity_mean[0],
                "VEL_Y_MEAN": velocity_mean[1],
                "VEL_Z_MEAN": velocity_mean[2],
                "MA_T_URMS": Ma_t_urms,
                "MA_T_QRMS": Ma_t_qrms,
                "MA_RMS": Ma_rms,
                "U_RMS": u_rms,
            ...
            "LENGTH_SCALES": {
                ...
                "LAMBDA_G": lambda_g,
                "KOLMOGOROV": eta,
                "KOLMOGOROV_RESOLUTION_1": eta_kmax,
                "KOLMOGOROV_RESOLUTION_2": eta_over_dx,
                "L_11": L_11, "L_22": L_22
            },
            "TIME_SCALES": {
                "TAU_LI": tau_LI,
            ...
            "REYNOLDS_NUMBERS": {
                ...
                "RE_LAMBDA_ENSTROPHY": Re_lambda_enstrophy,
                "RE_INTEGRAL": Re_int,
    ...
    def channel_statistics(
        ...
        V_mean = jnp.mean(V, axis=(0,1,3), keepdims=True)
        V_mean_y = jnp.squeeze(V_mean)
        W_mean = jnp.mean(W, axis=(0,1,3), keepdims=True)
        W_mean_y = jnp.squeeze(W_mean)
