...
class ALDM_WENO5(SpatialReconstruction):
    """ALDM_WENO5 
    # TODO should inherit from WENO5BASE
    ...
    def __init__(
        ...
        self.dr_aldm = [0.89548, 0.08550, 0.01902]
        self.dr_uniform = [0.1, 0.6, 0.3]
        self.cr_uniform = [
            [1/3, -7/6, 11/6], 
            [-1/6, 5/6, 1/3], 
            [1/3, 5/6, -1/6]
    ...
    def get_adaptive_ideal_weights(
        ...
        d0 = self.dr_aldm[0] + fs * (dr[0] - self.dr_aldm[0])
        d1 = self.dr_aldm[1] + fs * (dr[1] - self.dr_aldm[1])
        d2 = self.dr_aldm[2] + fs * (dr[2] - self.dr_aldm[2])
    ...
    def reconstruct_xi(
        ...
        if self.smoothness_measure == "TV":
            ...
            beta_0 = jnp.square(buffer[s_[1]] - buffer[s_[0]]) \
                   + jnp.square(buffer[s_[2]] - buffer[s_[1]])
            beta_1 = jnp.square(buffer[s_[2]] - buffer[s_[1]]) \
                   + jnp.square(buffer[s_[3]] - buffer[s_[2]])
            beta_2 = jnp.square(buffer[s_[3]] - buffer[s_[2]]) \
                   + jnp.square(buffer[s_[4]] - buffer[s_[3]])
        ...
        d0, d1, d2 = self.get_adaptive_ideal_weights(dr, j, fs)
