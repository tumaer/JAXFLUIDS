...
class WENO9HJ(SpatialDerivative):
    
    required_halos = 5

    def __init__(self, nh: int, inactive_axes: List, offset: int = 0):
        super(WENO9HJ, self).__init__(nh, inactive_axes, offset)
        self.derivative_stencil = DerivativeSecondOrderFace(nh, inactive_axes, offset + self.required_halos - 1)
        self.reconstruction_stencil = WENO9JS(offset + self.required_halos, inactive_axes, offset)
