...
class WENO6CU(SpatialReconstruction):
    ...
    def __init__(self, nh: int, inactive_axis: List) -> None:
        super(WENO6CU, self).__init__(nh=nh, inactive_axis=inactive_axis)
    ...
    def reconstruct_xi(self, buffer: jnp.ndarray, axis: int, j: int, dx: float = None, **kwargs) -> jnp.ndarray:
        ...
        alpha_0 = self.dr_[j][0] * (self.C_ + tau_6 / (beta_0 + self.eps) )
        alpha_1 = self.dr_[j][1] * (self.C_ + tau_6 / (beta_1 + self.eps) )
        alpha_2 = self.dr_[j][2] * (self.C_ + tau_6 / (beta_2 + self.eps) )
        alpha_3 = self.dr_[j][3] * (self.C_ + tau_6 / (beta_3 + self.eps) )
