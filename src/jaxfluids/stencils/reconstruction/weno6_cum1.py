...
class WENO6CUM1(SpatialReconstruction):
    ...
    def __init__(self, nh: int, inactive_axis: List) -> None:
        super(WENO6CUM1, self).__init__(nh=nh, inactive_axis=inactive_axis)
    ...
    def reconstruct_xi(self, buffer: jnp.DeviceArray, axis: int, j: int, dx: float, **kwargs) -> jnp.DeviceArray:
        ...
        alpha_0 = self.dr_[j][0] * jnp.power(( self.Cq_ + tau_6 / (beta_0 + self.eps * dx2) ), self.q_ )
        alpha_1 = self.dr_[j][1] * jnp.power(( self.Cq_ + tau_6 / (beta_1 + self.eps * dx2) ), self.q_ )
        alpha_2 = self.dr_[j][2] * jnp.power(( self.Cq_ + tau_6 / (beta_2 + self.eps * dx2) ), self.q_ )
        alpha_3 = self.dr_[j][3] * jnp.power(( self.Cq_ + tau_6 / (beta_3 + self.eps * dx2) ), self.q_ )
