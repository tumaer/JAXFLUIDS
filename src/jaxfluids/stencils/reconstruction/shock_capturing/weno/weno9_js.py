...
class WENO9JS(SpatialReconstruction):
    ''' Balsara & Shu - 2000 - '''
    def __init__(self,
            nh: int,
            inactive_axes: List,
            offset: int = 0,
            **kwargs) -> None:
        super(WENO9JS, self).__init__(nh=nh, inactive_axes=inactive_axes, offset=offset)

        self._dr = (1/126, 10/63, 10/21, 20/63, 5/126)
    ...
    def reconstruct_xi(self, buffer: Array, axis: int, j: int, dx: float = None,
        ...
        alpha_3 = self._dr[3] * one_beta_3_sq
        alpha_4 = self._dr[4] * one_beta_4_sq
        ...
        p_0 = self._cr[0][0] * buffer[s1_[0]] + self._cr[0][1] * buffer[s1_[1]] \
            + self._cr[0][2] * buffer[s1_[2]] + self._cr[0][3] * buffer[s1_[3]] \
            + self._cr[0][4] * buffer[s1_[4]]
        p_1 = self._cr[1][0] * buffer[s1_[1]] + self._cr[1][1] * buffer[s1_[2]] \
            + self._cr[1][2] * buffer[s1_[3]] + self._cr[1][3] * buffer[s1_[4]] \
            + self._cr[1][4] * buffer[s1_[5]]
        p_2 = self._cr[2][0] * buffer[s1_[2]] + self._cr[2][1] * buffer[s1_[3]] \
            + self._cr[2][2] * buffer[s1_[4]] + self._cr[2][3] * buffer[s1_[5]] \
            + self._cr[2][4] * buffer[s1_[6]]
        p_3 = self._cr[3][0] * buffer[s1_[3]] + self._cr[3][1] * buffer[s1_[4]] \
            + self._cr[3][2] * buffer[s1_[5]] + self._cr[3][3] * buffer[s1_[6]] \
            + self._cr[3][4] * buffer[s1_[7]]
        p_4 = self._cr[4][0] * buffer[s1_[4]] + self._cr[4][1] * buffer[s1_[5]] \
            + self._cr[4][2] * buffer[s1_[6]] + self._cr[4][3] * buffer[s1_[7]] \
            + self._cr[4][4] * buffer[s1_[8]]
