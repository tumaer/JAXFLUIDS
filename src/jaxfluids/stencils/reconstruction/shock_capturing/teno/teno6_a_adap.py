...
class TENO6AADAP(SpatialReconstruction):
    ...
    def reconstruct_xi(
        ...
        eta_0 = (jnp.abs(2 * (buffer[s_[2]] - buffer[s_[1]]) * (buffer[s_[1]] - buffer[s_[0]])) + self.eps_dissipation) \
            / ((buffer[s_[2]] - buffer[s_[1]])**2 + (buffer[s_[1]] - buffer[s_[0]])**2 + self.eps_dissipation) 
        eta_1 = (jnp.abs(2 * (buffer[s_[3]] - buffer[s_[2]]) * (buffer[s_[2]] - buffer[s_[1]])) + self.eps_dissipation) \
            / ((buffer[s_[3]] - buffer[s_[2]])**2 + (buffer[s_[2]] - buffer[s_[1]])**2 + self.eps_dissipation)
        eta_2 = (jnp.abs(2 * (buffer[s_[4]] - buffer[s_[3]]) * (buffer[s_[3]] - buffer[s_[2]])) + self.eps_dissipation) \
            / ((buffer[s_[4]] - buffer[s_[3]])**2 + (buffer[s_[3]] - buffer[s_[2]])**2 + self.eps_dissipation)
        eta_3 = (jnp.abs(2 * (buffer[s_[5]] - buffer[s_[4]]) * (buffer[s_[4]] - buffer[s_[3]])) + self.eps_dissipation) \
            / ((buffer[s_[5]] - buffer[s_[4]])**2 + (buffer[s_[4]] - buffer[s_[3]])**2 + self.eps_dissipation)

        eta = jnp.minimum(jnp.minimum(eta_0, eta_1), jnp.minimum(eta_2, eta_3))
        ...
        w0 = dr[0] * jnp.where(gamma_0 * one_gamma_sum < CT, 0, 1)
        w1 = dr[1] * jnp.where(gamma_1 * one_gamma_sum < CT, 0, 1)
        w2 = dr[2] * jnp.where(gamma_2 * one_gamma_sum < CT, 0, 1)
        w3 = dr[3] * jnp.where(gamma_3 * one_gamma_sum < CT, 0, 1)
