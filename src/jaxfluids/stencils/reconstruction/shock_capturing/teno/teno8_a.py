...
class TENO8A(SpatialReconstruction):
    ...
    def reconstruct_xi(self, 
        ...
        w0 = self.dr_[0] * jnp.where(gamma_0 * one_gamma_sum < CT, 0, 1)
        w1 = self.dr_[1] * jnp.where(gamma_1 * one_gamma_sum < CT, 0, 1)
        w2 = self.dr_[2] * jnp.where(gamma_2 * one_gamma_sum < CT, 0, 1)
        w3 = self.dr_[3] * jnp.where(gamma_3 * one_gamma_sum < CT, 0, 1)
        w4 = self.dr_[4] * jnp.where(gamma_4 * one_gamma_sum < CT, 0, 1)
        w5 = self.dr_[5] * jnp.where(gamma_5 * one_gamma_sum < CT, 0, 1)
