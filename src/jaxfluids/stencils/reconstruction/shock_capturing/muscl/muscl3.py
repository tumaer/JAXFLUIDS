...
class MUSCL3(SpatialReconstruction):
    ...
    def reconstruct_xi(self, 
        ...
        # r = (buffer[s_[1]] - buffer[s_[0]]) / (self.eps + (buffer[s_[2]] - buffer[s_[1]])) #2.28
        # limiter = self.limiter(r)
        # cell_state_xi_j = buffer[s_[1]] + 0.5 * limiter * (buffer[s_[2]] - buffer[s_[1]]) #2.31
        ...
        if j == 0:
            ...
            limiter = self.limiter(r)
            cell_state_xi_j = buffer[s_[1]] + 0.5 * limiter * delta_upwind     #2.31
        if j == 1:
