...
class WENO3FP(SpatialReconstruction):
    ...
    def reconstruct_xi(self, buffer: jnp.ndarray, axis: int, j: int, dx: float = None, **kwargs) -> jnp.ndarray:
        ...
        alpha_0 = self.dr_[j][0] * (1.0 + (tau_f3 + self.eps) / (beta_0 + self.eps) + gamma_3 * (beta_0 + self.eps) / (tau_f3 + self.eps) )
        alpha_1 = self.dr_[j][1] * (1.0 + (tau_f3 + self.eps) / (beta_1 + self.eps) + gamma_3 * (beta_1 + self.eps) / (tau_f3 + self.eps) )
