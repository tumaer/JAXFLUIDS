...
class DiffuseFiveEquationMixture(Mixture):
    """DiffuseFiveEquationMixture
    ...
    def compute_mixture_EOS_params(self, alpha_i: Array) -> Tuple[Array, Array]:
        ...
        gamma_mixture = 1.0 + 1.0 / sum(alpha_i[ii] * self.one_gamma_[ii] for ii in range(len(self.one_gamma_)))
        pb_mixture = (gamma_mixture - 1.0) / gamma_mixture * sum(alpha_i[ii] * self.gamma_pb_[ii] for ii in range(len(self.gamma_pb_)))
    ...
    def get_thermal_conductivity(
        ...
        thermal_conductivity = jnp.stack([material.get_thermal_conductivity(temperature) for material in self.materials.values()], axis=0)
    ...
    def get_dynamic_viscosity(
        ...
        alpha_i = jnp.stack([*alpha_i, 1.0 - jnp.sum(alpha_i, axis=0)], axis=0)
        # TODO consider substituting with for loop instead of stacking
        dynamic_viscosity = jnp.stack([material.get_dynamic_viscosity(temperature) for material in self.materials.values()], axis=0)
    ...
    def get_phasic_volume_specific_enthalpy(self, p: Array) -> Array:
        ...
        \rho h = \gamma (p + \pi_{\infty}) / (\gamma - 1)
